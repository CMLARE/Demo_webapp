...
neuralNetworkWetDryPath = "cmlare/trained_models/mlp_ANN_wet_dry h-13 RSL_MIN,RSL_MAX,RSL_AVG,TSL_MAX L-100.sav"
neuralNetworkWetDryPath= os.path.join(BASE_DIR, neuralNetworkWetDryPath)

neuralNetworkWetDry = pickle.load(open(neuralNetworkWetDryPath, "rb"))
...
def wetDryClassification(dataframe):
    ...
    StandardScaler(copy=True, with_mean=True, with_std=True)
    X = scaler.transform(X)
    predictions = neuralNetworkWetDry.predict(X)
